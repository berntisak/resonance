...
namespace LaptopOrchestra.Kinect.Model
{
    public class TabWindowViewModel: ViewModelBase
    {
        ...
        public TabWindowViewModel(SessionManager sessionManager)
        {
            ...
            AllJoints = new string[Constants.MaxSessions, Constants.MaxJoints];
        ...
        private void UpdateFirstArray()
        {
            ...
            FirstArray = new string[Constants.MaxSessions];
            ...
            foreach (SessionWorker sw in workers)
            {
                ...
                foreach (JointType jt in jointTypes) {
                    if (flags.JointFlags[jt][1])
                    {
                        MyJointList.Add("Body 1: " + jt.ToString());
                    }
                    if (flags.JointFlags[jt][2])
                    {
                        MyJointList.Add("Body 2: " + jt.ToString());
                    }
                }
        ...
        private void UpdateFirstList()
        {
            ...
            //Find Tab to show
            for (y = 0; y < Constants.MaxSessions; y++)
            {
                if (SelectedTab == null)
                {
                    z = 0;
        ...
        private void UpdateSecondList(string id)
        {
            ...
            {   
                ...
                string[] TempArray = new string[Constants.MaxJoints];
                for (int i = 0; i<Constants.MaxJoints; i++)
                {
